...
plt.savefig('savedPlots_checked/cprob_wholeRun_exp1.pdf')
...
plt.savefig('savedPlots_checked/cprob__context_wholeRun_exp1.pdf')
...
fig = plotPosterStyle_multiplePTS(prob_cheating_pre_post,subjects)
all_correct_prob_pre_post = prob_cheating_pre_post.copy()
all_correct_prob_pre_post[P_ind,:] = 1 - prob_cheating_pre_post[P_ind,:]
subj_diff_correct_pre_post = np.diff(all_correct_prob_pre_post,axis=1)
fig,ax = plt.subplots(figsize=(20,9))
plt.subplot(2,1,1)
sns.despine()
plt.errorbar(
	x=np.arange(3),
	y=np.nanmean(subj_diff_correct,axis=0),
	yerr=scipy.stats.sem(subj_diff_correct,
	axis=0,nan_policy='omit'
	),
	color='k',
	alpha=0.7,
...
# all_correct_prob_z = stats.zscore(all_correct_prob,axis=1,ddof=1)
# prob_cheating_diff = np.diff(prob_cheating, axis=1)
...
# # run 3 - run 1
# end_diff_run = prob_cheating[:,1,3] - prob_cheating[:,1,0]
# plotPosterStyle_DF(end_diff_run,subjects)


# correct_diff_run = end_diff_run.copy()
# correct_diff_run[P_ind] = -1*end_diff_run[P_ind]
# data = {}
...
# plt.errorbar(
# 	x=np.arange(4),
# 	y=np.nanmean(all_correct_prob_z[top_subjects,1,:],axis=0),
...
# plt.errorbar(
# 	x=np.arange(4),
# 	y=np.nanmean(all_correct_prob_z[bottom_subjects,1,:],axis=0),
