%% specify any input
debug = 1;
saveDir = '/Volumes/norman/amennen/RT_prettymouth/testdata';
subjectNum = 1;
...
CORRECT_ANSWER(1) = 1;
QUESTIONS{2} = 'What was the gray-haired man name?';
...
CORRECT_ANSWER(10) = 1;
QUESTIONS{11} = 'What did Lee advice Arthur to do?';
...
POSSIBLE_ANSWERS{26,2} = 'she was with another men.';
QUESTIONS{27} = 'What did you think - Why did Joanie come back home so late?';
POSSIBLE_ANSWERS{27,1} = 'She was with another men';
...
POSSIBLE_ANSWERS{36,2} = 'other man';
QUESTIONS{37} = 'When the phone rang at the first time, why did you think the gray-haired man ask the girl if she would rather he didn?t answer it?';
...
basicInstruct{1} = 'You will now be asked to answer questions about the story you just heard.';
basicInstruct{2} = 'Please do you best to answer the questions as accurately and honestly as possible.';
basicInstruct{3} = 'Hit the UP arrow to choose the top response, and hit the DOWN arrow to choose the bottom response.';
basicInstruct{4} = 'Press the UP arrow when you are ready to begin.';
...
for iTrial = 1:nQ
    ...
    questionText{2} = {iTrial}upResponse;
    questionText{3} = {iTrial}downResponse;
    
    for i=1:length(questionText)
        tempBounds = Screen('TextBounds',mainWindow,questionText{i});
        Screen('drawtext',mainWindow,questionText{i},centerX-tempBounds(3)/2,centerY-tempBounds(4)+1.5*textSpacing*(i-1),textColor);
        clear tempBounds;
    end
...
Screen('Flip',mainWindow);
%% save
save([saveDir '/test1' '_' datestr(now,30)],'stim', 'timing');
